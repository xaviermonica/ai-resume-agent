...
function resolvePythonExec(): string {
  // Prefer explicit env override, then venv python, fallback to system python3
  const fromEnv = process.env.PYTHON_PATH
  if (fromEnv && fromEnv.length > 0) return fromEnv
  const venvPy = '.venv/bin/python3'
...
function runPythonAgent(payload: any, env: NodeJS.ProcessEnv): Promise<{ ok: boolean; data?: any; error?: string }> {
  return new Promise((resolve) => {
    const pyExec = resolvePythonExec()
    const py = spawn(pyExec, ['agent/agent.py'], {
      env: {
        ...process.env,
        ...env,
        // Allow importing vendored python packages if present
        PYTHONPATH: process.env.PYTHONPATH || 'agent/.python_packages',
      },
